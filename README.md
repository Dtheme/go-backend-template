...
7. Write real results back into the plan, architecture doc, Postman collection and smoke script, and record in `.ai/memory.md` anything the next session would otherwise rediscover.
...
| Memory | `.ai/memory.md`: verified facts, mistakes already made, judgements not yet proven; `spec-check` requires the file and its three headings |
...
25 articles walking through every mechanism, in Chinese, plus a reading guide and a cheat sheet: [docs/guide/](docs/guide/), or read them online on [Feishu](https://ncnhrkchbfwf.feishu.cn/wiki/SVHxwROhfiYBE2k2I35cJ0IUnYe).
